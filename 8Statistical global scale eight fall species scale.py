...
for name in ['BIRD', 'MAMMALS', 'REPTILES', 'PLANTS']:
    # 读取第一个栅格文件
    with rasterio.open(f'E:/Datasatlast/LossResults/SSP2_RCP45/SSP2_RCP45_2050_{name}.tif') as src1:
    ...
    mask_negative = (raster1 < -0.9) & (raster1 != nodata)

    # 读取第二个栅格文件
    with rasterio.open('classified/classified_all.tif') as src2:
    ...
    data = {
        ...
        'Count_Positive': [count_positive.get(i, 0)/10000 for i in range(1, 9)],
        'Count_Zero': [count_zero.get(i, 0)/10000 for i in range(1, 9)],
        'Count_Negative': [count_negative.get(i, 0)/10000 for i in range(1, 9)]
    }

    df = pd.DataFrame(data)
    df.to_csv(f'table3statistic/output_counts_{name}.csv', index=False)

    print("结果已保存到"+ f'table3statistic/output_counts_{name}.csv')